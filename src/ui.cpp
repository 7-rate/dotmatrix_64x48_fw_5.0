...
public:
	screen_message_box_t(const String &_title, const string_vector &_lines) : title(_title), lines(_lines)
	{
	}
...
protected:
	...
	{
		if (initial)
			return; // ignore button input before once all buttons get released
		...
		case BUTTON_RIGHT:
			...
			if (x < 0)
				x = 0;
			if (x >= LED_MAX_LOGICAL_COL)
				x = LED_MAX_LOGICAL_COL - 1;
		...
		case BUTTON_DOWN:
			...
			if (y < 0)
				y = 0;
			if (y >= LED_MAX_LOGICAL_ROW)
				y = LED_MAX_LOGICAL_ROW - 1;
			get_current_frame_buffer().fill(0);
...
public:
	screen_ascii_editor_t(const String &_title, const String &_line = "", int _max_chars = -1) : title(_title),
																								 line(_line),
...
{
public:
	screen_ip_editor_t(const String &_title, const String &_line = "") : screen_ascii_editor_t(_title, _line, 15) // XXX.XXX.XXX.XXX = 15 chars
	{
...
public:
	screen_menu_t(const String &_title, const string_vector &_items) : title(_title), items(_items)
	{
	}
...
public:
	screen_menu_with_marquee_t(
		...
		const string_vector &_items) : screen_menu_t(_title, _items)
	{
		title_line_y += 6;
		list_start_y += 6;
		--max_lines;
...
public:
	screen_dns_2_editor_t(const ip_addr_settings_t &_settings) : screen_ip_editor_t(F("DNS Srvr 2"), _settings.dns2), settings(_settings) {}
...
public:
	screen_dns_1_editor_t(const ip_addr_settings_t &_settings) : screen_ip_editor_t(F("DNS Srvr 1"), _settings.dns1), settings(_settings) {}
...
public:
	screen_net_mask_editor_t(const ip_addr_settings_t &_settings) : screen_ip_editor_t(F("Net Mask"), _settings.ip_mask), settings(_settings) {}

protected:
	bool validate(const String &line) override
	{
		IPAddress addr;
		if (!addr.fromString(line))
			return false; // IPAddress::fromString validates the string
...
public:
	screen_ip_gateway_editor_t(const ip_addr_settings_t &_settings) : screen_ip_editor_t(F("Gateway"), _settings.ip_gateway), settings(_settings) {}
...
public:
	screen_ip_addr_editor_t(const ip_addr_settings_t &_settings) : screen_ip_editor_t(F("IP Addr"), _settings.ip_addr), settings(_settings) {}
...
public:
	screen_ap_pass_editor_t(const String &_ap_name) : screen_ascii_editor_t(F("AP Pass"), wifi_get_ap_pass()), ap_name(_ap_name)
	{
	}
...
private:
	...
	void draw_clock()
	{
		...
		buf[0] = 0xE2;
		buf[1] = 0x82;
		buf[2] = tm.tm_sec / 10 + 0x80;
		buf[3] = 0xE2;
		buf[4] = 0x82;
		buf[5] = tm.tm_sec % 10 + 0x80;
		buf[6] = 0;
